...
static void pic_init(void)
{
    ...
    // outb (PCI_MASTER_DATA_PORT, 0xfd);
    // outb (PCI_SLAVE_DATA_PORT, 0xff);
...
static void generate_interrupt_handler(uint8_t nvec)
{
    ...
    ccos_puts(interrupt_name[nvec]);
    ccos_puts("\n\n");
    if (nvec == PAGE_FAULT_NUM) {	  // 若为Pagefault,将缺失的地址打印出来并悬停
        int page_fault_vaddr = 0; 
        asm ("movl %%cr2, %0" : "=r" (page_fault_vaddr));	  // cr2是存放造成page_fault的地址
        ccos_puts("\npage fault occurs, with addr is 0x");__ccos_display_int(page_fault_vaddr);ccos_puts("\n\n"); 
    }
