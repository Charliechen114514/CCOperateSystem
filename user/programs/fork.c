...
static int32_t copy_pcb_vaddrbitmap_stack0(TaskStruct *child_thread,
    ...
    memcpy(child_thread, parent_thread,
           PGSIZE);                 // Copy parent process's PCB information
        ...
        if (vaddr_btmp[idx_byte]) { // If the page is allocated in the bitmap
            ...
            while (idx_bit < 8) {
                if ((BITMAP_MASK << idx_bit) & vaddr_btmp[idx_byte]) {
                    prog_vaddr =
                        (idx_byte * 8 + idx_bit) * PGSIZE + vaddr_start;
                    ...
                    memcpy(buf_page, (void *)prog_vaddr, PGSIZE);
                     ...
                     * user space */
                    memcpy((void *)prog_vaddr, buf_page, PGSIZE);
...
static int32_t build_child_stack(TaskStruct *child_thread) {
    ...
    Interrupt_Stack *intr_0_stack =
        (Interrupt_Stack *)((uint32_t)child_thread + PGSIZE -
...
static void update_inode_open_cnts(TaskStruct *thread) {
    ...
    while (local_fd < MAX_FILES_OPEN_PER_PROC) {
        ...
        if (global_fd != -1) {
            if (is_pipe(local_fd)) {
                file_table[global_fd]
                    .fd_pos++; // Increment pipe file descriptor position
            } else {
                file_table[global_fd]
                    .fd_inode->i_open_cnts++; // Increment inode open count for
                                              // regular files
            }
        }
        local_fd++;
