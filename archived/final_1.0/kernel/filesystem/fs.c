...
DiskPartition *cur_part; // 默认情况下操作的是哪个分区
...
static bool mount_partition(list_elem *pelem, int arg) {
    ...
    /* If the partition name matches, mount the partition */
    if (!strcmp(part->name, part_name)) {
        cur_part = part; // Set the current partition
        Disk *hd = cur_part->my_disk;

        /* sb_buf is used to store the superblock read from the hard disk */
        struct super_block *sb_buf =
            (struct super_block *)sys_malloc(SECTOR_SIZE);

        /* Create the superblock for the current partition in memory */
        cur_part->sb =
            (struct super_block *)sys_malloc(sizeof(struct super_block));
        if (cur_part->sb == NULL) {
            KERNEL_PANIC("alloc memory failed!");
        }
        ...
        /* Copy the superblock information from sb_buf into the partition's
         * superblock */
        memcpy(cur_part->sb, sb_buf, sizeof(struct super_block));

        /**********     Read the block bitmap from the hard disk into memory
         * ****************/
        ...
        /* Read the block bitmap from the hard disk into the partition's
         * block_bitmap.bits */
        ide_read(hd, sb_buf->block_bitmap_lba, cur_part->block_bitmap.bits,
                 sb_buf->block_bitmap_sects);
        /*************************************************************/

        /**********     Read the inode bitmap from the hard disk into memory
         * ************/
        ...
        /* Read the inode bitmap from the hard disk into the partition's
         * inode_bitmap.bits */
        ide_read(hd, sb_buf->inode_bitmap_lba, cur_part->inode_bitmap.bits,
                 sb_buf->inode_bitmap_sects);
        /*************************************************************/

        /* Initialize the list for open inodes */
        list_init(&cur_part->open_inodes);
        verbose_printk("mount %s done!\n", part->name);

        /* Return true to stop the list_traversal function, preventing
         * unnecessary traversal of subsequent elements */
        return true;
    }
    return false; // Continue the list_traversal if partition is not found
}
...
char *path_parse(char *pathname, char *name_store) {
    if (pathname[0] == '/') { // No need to parse the root directory separately
    ...
    if (pathname[0] == 0) { // If the path string is empty, return NULL
        return NULL;
    }
    return pathname;
}
...
int32_t path_depth_cnt(char *pathname) {
    ...
    while (name[0]) {
        ...
        if (p) { // If p is not NULL, continue analyzing the path
            p = path_parse(p, name);
        }
    }
    return depth;
...
static int search_file(const char *pathname,
    ...
    while (name[0]) { // If the first character is the end of the string, stop
                      // the loop
        ...
        if (search_dir_entry(cur_part, parent_dir, name, &dir_e)) {
            ...
            if (FT_DIRECTORY == dir_e.f_type) { // If the item is a directory
                ...
                continue;
                       ...
                       dir_e.f_type) { // If the item is a regular file
...
int32_t sys_unlink(const char *pathname) {
    ...
    if (file_idx < MAX_FILE_OPEN) {
        dir_close(searched_record.parent_dir);
        printk("file %s is in use, cannot delete!\n", pathname);
        return -1;
    }
...
int32_t sys_mkdir(const char *pathname) {
    ...
    if (inode_no != -1) { // If a file or directory with the same name exists,
                          // return failure
        printk("sys_mkdir: file or directory %s exist!\n", pathname);
        rollback_step = 1;
        goto rollback;
    } else { // If not found, check whether the final directory exists or if any
         ...
         */
        if (pathname_depth !=
            path_searched_depth) { // This means some intermediate directory is
                                   // missing
    ...
    if (!sync_dir_entry(parent_dir, &new_dir_entry,
                        io_buf)) { // sync_dir_entry will synchronize the block
                                   // bitmap to disk
...
Dir *sys_opendir(const char *name) {
    ...
    Dir *ret = NULL;
    if (inode_no ==
        -1) { // If the directory is not found, print an error message
...
static int get_child_dir_name(uint32_t p_inode_nr, uint32_t c_inode_nr,
    ...
    if (parent_dir_inode
            ->i_sectors[12]) { // If there is a single indirect block table,
    ...
    while (block_idx < block_cnt) {
        if (all_blocks[block_idx]) { // If the block is not empty, read it
...
char *sys_getcwd(char *buf, uint32_t size) {
    ...
    while ((child_inode_nr)) {
        ...
        if (get_child_dir_name(
                ...
                io_buf) == -1) { // If name not found, fail and exit
            sys_free(io_buf);
            return NULL;
        }
...
int32_t sys_stat(const char *path, struct stat *buf) {
    ...
    memset(
        &searched_record, 0,
        sizeof(struct path_search_record)); // Initialize or clear, otherwise
...
void filesys_init() {
    ...
    while (channel_no < channel_cnt) {
        dev_no = 0;
        while (dev_no < 2) {
            if (dev_no == 0) { // Skip raw disk hd60M.img
                dev_no++;
                continue;
            }
            ...
            while (part_idx <
                   12) { // 4 primary partitions + 8 logical partitions
                if (part_idx == 4) { // Start processing logical partitions
                    part = hd->logic_parts;
                }
                ...
                if (part->sec_cnt != 0) { // If the partition exists
                    memset(sb_buf, 0, SECTOR_SIZE);
