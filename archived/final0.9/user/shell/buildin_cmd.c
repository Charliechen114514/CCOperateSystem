...
static void wash_path(char *old_abs_path, char *new_abs_path) {
    assert(old_abs_path[0] == '/'); // Ensure the path starts with "/"
    ...
    while (name[0]) {
        ...
        } else if (strcmp(".", name)) { // If the path is not ".", append it to
                                        // new_abs_path
...
void buildin_ls(uint32_t argc, char **argv) {
    ...
    while (arg_idx < argc) {
        if (argv[arg_idx][0] == '-') { // If it's an option, starting with "-"
            if (!strcmp("-l", argv[arg_idx])) { // "-l" option for long format
                long_info = true;
            } else if (!strcmp("-h", argv[arg_idx])) { // "-h" option for help
