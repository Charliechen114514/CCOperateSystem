...
static void partition_format(DiskPartition *part)
{
    ...
    verbose_printk("%s info:\n", part->name);
    verbose_printk("   magic:0x%x\n   part_lba_base:0x%x\n   all_sectors:0x%x\n   "
    ...
    verbose_printk("   super_block_lba:0x%x\n", part->start_lba + 1);
    ...
    verbose_printk("   root_dir_lba:0x%x\n", sb.data_start_lba);
    verbose_printk("%s format done\n", part->name);
    sys_free(buf);
}
...
{
    KERNEL_ASSERT(k_strlen(pathname) < MAX_PATH_LEN);
...
{
    KERNEL_ASSERT(k_strlen(name) < MAX_PATH_LEN);
