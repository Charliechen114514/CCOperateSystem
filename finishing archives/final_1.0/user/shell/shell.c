...
#define MAX_ARG_NR 16 // Maximum of 15 arguments plus the command name
...
static void readline(char *buf, int32_t count) {
    ...
    while (read(stdin_no, pos, 1) != -1 &&
           (pos - buf) < count) { // Read until enter key is found
        switch (*pos) {
        ...
        case '\b':
            if (cmd_line[0] != '\b') { // Prevent deleting non-inputted data
...
static int32_t cmd_parse(char *cmd_str, char **argv, char token) {
    ...
    while (*next) {
        ...
        while (*next &&
               *next != token) { // Find the delimiter before the string ends
            next++;
        }
...
void ccos_baseshell(void) {
    ...
    while (1) {
        ...
        memset(final_path, 0, MAX_PATH_LEN); // Clear the final path buffer
        memset(cmd_line, 0, MAX_PATH_LEN);   // Clear the command line buffer
        ...
        char *pipe_symbol =
            strchr(cmd_line, '|'); // Check for a pipe symbol '|'
        if (pipe_symbol) {
            ...
            pipe_symbol = strchr(each_cmd, '|'); // Find the first pipe symbol
            *pipe_symbol =
    ...
    panic("ccos_baseshell: should not be here"); // This should never be reached
}
