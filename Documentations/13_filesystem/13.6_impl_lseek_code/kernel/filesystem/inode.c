...
static void inode_locate(DiskPartition *part, uint32_t inode_no, InodePosition *inode_pos)
{
   ...
   { // If the remaining space in the sector is insufficient to hold the inode, it spans two sectors
      inode_pos->two_sec = true;
   }
   else
   { // Otherwise, the inode fits within a single sector
...
{ // io_buf is the buffer for disk IO
   ...
   {                                                            // If the inode spans two sectors, read and write two sectors
...
Inode *inode_open(DiskPartition *part, uint32_t inode_no)
{
   ...
   char *inode_buf;
   if (inode_pos.two_sec)
   { // Handle the case where the inode spans two sectors
   ...
   { // Otherwise, the inode does not span sectors, so a single sector buffer is enough
