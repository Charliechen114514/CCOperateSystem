...
​	`inode_delete` 和 `inode_release` 函数用于管理磁盘上的 inode 和数据块，确保文件删除时，磁盘资源得到适当回收。`inode_delete` 函数的作用是清除指定 inode 的数据。首先，它通过 `KERNEL_ASSERT` 检查 inode 编号是否有效。接着，调用 `inode_locate` 定位 inode 在磁盘上的位置，确保其所在的扇区地址在有效范围内。如果 inode 跨越两个扇区，则读取这两个扇区的数据，并通过 `k_memset` 将 inode 数据清空后写回磁盘。如果 inode 只占用一个扇区，则仅读取一个扇区，清除数据后再写回。`inode_release` 函数的目标是释放 inode 占用的所有资源，包括数据块和 inode 本身。首先，它通过 `inode_open` 打开 inode，并通过 `KERNEL_ASSERT` 确保 inode 编号匹配。接着，函数将 inode 的 12 个直接块地址存入 `all_blocks` 数组，如果存在一级间接块，则读取其内容并将其存入数组。函数会遍历 `all_blocks` 中的所有块地址，将对应的数据块从块位图中释放。接下来，释放 inode 本身所占用的 inode 位图块，最后通过调用 `inode_delete` 清除 inode 的磁盘内容，虽然这一部分只是调试用，并不影响实际的 inode 管理。

## 删除目录

文件名在文件系统中是通过目录项的方式存在的，因此删除文件时，必须先在相应目录中擦除该文件的目录项。删除目录项的过程包括以下步骤：

首先，在文件所在的目录中，系统会擦除该文件的目录项，将其内容置为 0。这样一来，文件的名字就不再在该目录中存在。

其次，根目录必须保留，因为它是文件系统结构的基础。根目录不会被删除，但可能会发生回收块的情况。如果某个目录项占用了一个单独的块，并且该块不是根目录的最后一个块，那么系统会将这个块回收，以释放空间。

接下来，目录 inode 中的 `i_size`（即目录大小）记录了目录项的总和。删除一个目录项后，系统会将 `i_size` 减去一个目录项的大小，以确保目录的大小与实际内容保持一致。

最后，目录的 inode 发生改变后，系统会将该变化同步到硬盘中，确保数据持久化。
