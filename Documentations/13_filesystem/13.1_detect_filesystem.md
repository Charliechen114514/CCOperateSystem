...
​	为了提高效率，微软推出了 NTFS 文件系统。与 FAT 文件系统不同，UNIX 操作系统使用一种名为 inode 的索引结构来组织文件。采用索引结构的文件系统虽然文件中的块仍然可以分散存储，但每个文件都配备了一个索引表，这个索引表存储了文件所有块的地址。通过索引表，访问文件的块变得更为高效，因为无需遍历链式结构，只需从索引表中获得块的地址即可。
