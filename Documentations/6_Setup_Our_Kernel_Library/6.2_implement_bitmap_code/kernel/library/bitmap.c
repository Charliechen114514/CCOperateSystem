...
{
    k_memset(btmp->bits, 0, btmp->btmp_bytes_len); // Set all bits to 0 (clear the bitmap)
}
...
bool bitmap_scan_test(Bitmap *btmp, uint32_t bit_idx)
{
    ...
    uint32_t bit_odd = bit_idx % 8;                           // Remainder gives the bit position within the byte
...
int bitmap_scan(Bitmap *btmp, uint32_t cnt)
{
    ...
    {
        /* 0xff means all bits in this byte are set (1), so move to the next byte */
        idx_byte++;
    }

    if (idx_byte >= btmp->btmp_bytes_len)
    { // If no free space is found in the entire bitmap
        return -1;
    }
    ...
    {
        idx_bit++; // Find the first free bit in the byte
    }

    int bit_idx_start = idx_byte * 8 + idx_bit; // Starting index of the free bit
    if (cnt == 1)
    {
        return bit_idx_start; // If only 1 free bit is requested, return immediately
    }

    uint32_t bit_left = (btmp->btmp_bytes_len * 8 - bit_idx_start); // Bits left to check
    uint32_t next_bit = bit_idx_start + 1;
    uint32_t count = 1; // Count of consecutive free bits found

    bit_idx_start = -1; // Initialize to -1; if no consecutive bits are found, return -1
    while (bit_left-- > 0)
    {
        if (!(bitmap_scan_test(btmp, next_bit)))
        { // If the next bit is free (0)
            count++;
        }
        else
        {
        ...
        if (count == cnt)
        {                                       // If the required number of consecutive free bits is found
            bit_idx_start = next_bit - cnt + 1; // Set the start index of the found block
            break;
        }
        next_bit++;
    }
    return bit_idx_start; // Return the starting bit index of the found block
}

/* Set the bit at bit_idx to the given value (0 or 1) */
void bitmap_set(Bitmap *btmp, uint32_t bit_idx, int8_t value)
{
    KERNEL_ASSERT((value == 0) || (value == 1)); // KERNEL_ASSERT that value is either 0 or 1
